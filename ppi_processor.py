...
class PPIProcessor:
    def __init__(self, geometry, config, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        na, nrb, mrm = self.c['num_angles'], self.c['num_range_bins'], self.c['max_range_m']
        self.na, self.nrb, self.mrm, self.bf = na, nrb, mrm, self.c.get('beamformer', 'conventional')

        self.ad = np.linspace(-90, 90, na)
        self.sv = geometry.compute_steering_vector(torch.tensor(self.ad * np.pi / 180, dtype=torch.float32, device=self.d)).to(self.d)
        self.win = torch.hann_window(nrb, periodic=False, dtype=torch.float32, device=self.d)
        self.map = torch.zeros((na, nrb), dtype=torch.float32, device=self.d)

    def _mvdr_weights(self, x, sv_rx):
        N = x.shape[1]
        r11, r12, r22 = torch.sum(x[0]*x[0].conj())/N, torch.sum(x[0]*x[1].conj())/N, torch.sum(x[1]*x[1].conj())/N
        reg = 1e-6
        r21 = r12.conj()
        det = (r11+reg)*(r22+reg) - r12*r21
